...
def _apply_acceleration(speed, speed_limit, forward=True):
    """Изменяет скорость с учётом ускорения/торможения."""
    ...
    delta = math.cos(speed_fraction) * ACCELERATION_FACTOR
    ...
    if abs(result_speed) < STOP_SPEED_THRESHOLD:
        result_speed = 0
...
def update_speed(
    ...
    """Обновляет скорость корабля с учётом направления и трения.

    Args:
        row_speed (float): текущая скорость по строкам.
        column_speed (float): текущая скорость по колонкам.
        rows_direction (int): -1 (вверх), 0, 1 (вниз).
        columns_direction (int): -1 (влево), 0, 1 (вправо).
        row_speed_limit (float): максимальная скорость по строкам.
        column_speed_limit (float): максимальная скорость по колонкам.
        fading (float): коэффициент трения (0..1).

    Returns:
        tuple: (new_row_speed, new_column_speed)
    """
